...
object WebLinksOpenApi extends App {
  ...
  val weblink = requests.post(s"https://www.growingio.com/api/v1/projects/$projectUid/meta/weblinks", data = web, headers = postHeaders)
  val weblinkPrettyFormatStr = AuthUtils.jsonFormat(weblink.text())
  println(weblinkPrettyFormatStr)

  //查询
  val wb = requests.get(s"https://www.growingio.com/api/v1/projects/$projectUid/meta/weblinks", headers = getHeaders)
